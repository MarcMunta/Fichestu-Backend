...
UPDATE tokens
SET current_price = ROUND((1 + (RANDOM() * 499))::numeric, 2),
    last_update = CURRENT_TIMESTAMP
...
FROM users u
CROSS JOIN tokens t
WHERE LOWER(t.name) NOT IN ('ficha gris', 'ficha incolora')
...
SELECT DATE '2000-01-03', 'Europe/Madrid', 'Reset inicial: 10 de cada ficha de mercado y 200 fichas grises por usuario.'
WHERE NOT EXISTS (
    SELECT 1
    FROM market_reset_audit
    WHERE business_date = DATE '2000-01-03'
);
